...
export class Sidebar {
  ...
  get currentNavigationItems(): NavigationItem[] {
    return this.navigationItems.length > 0 ? this.navigationItems : this.defaultNavigationItems;
  }
  ...
  expandParentIfChild(targetItem: NavigationItem): void {
    this.currentNavigationItems.forEach(item => {
      if (item.children) {
        const isChildActive = item.children.some(child => child.id === targetItem.id);
        if (isChildActive) {
